import { ButtonGroup } from "@/components/ui/button-group";
import { DataTable } from "./_component/data-table";
import { columns, Order } from "./_component/table-columns";
import { Button } from "@/components/ui/button";
import { PlusIcon } from "lucide-react";
import { LuFilter } from "react-icons/lu";
import { IoIosArrowDown } from "react-icons/io";
import { RiResetRightFill } from "react-icons/ri";

async function getData(): Promise<Order[]> {
  return [
    {
      id: "00001",
      clientName: "Apple Watch",
      clientAddress: "6096 Marjolaine Landing",
      dateTime: new Date("2024-01-15"),
      type: "Electric",
      status: "Completed",
    },
    {
      id: "00002",
      clientName: "Apple Watch",
      clientAddress: "6096 Marjolaine Landing",
      dateTime: new Date("2024-01-15"),
      type: "Electric",
      status: "Completed",
    },
    {
      id: "00003",
      clientName: "Apple Watch",
      clientAddress: "6096 Marjolaine Landing",
      dateTime: new Date("2024-01-15"),
      type: "Electric",
      status: "Completed",
    },
    {
      id: "00004",
      clientName: "Apple Watch",
      clientAddress: "6096 Marjolaine Landing",
      dateTime: new Date("2024-01-15"),
      type: "Electric",
      status: "Completed",
    },
    {
      id: "00005",
      clientName: "Apple Watch",
      clientAddress: "6096 Marjolaine Landing",
      dateTime: new Date("2024-01-15"),
      type: "Electric",
      status: "Completed",
    },
    {
      id: "00006",
      clientName: "Apple Watch",
      clientAddress: "6096 Marjolaine Landing",
      dateTime: new Date("2024-01-15"),
      type: "Electric",
      status: "Completed",
    },
  ];
}

export default async function DashboardOrderListView() {
  const data = await getData();

  return (
    <div>
      <h1 className="font-semibold text-2xl lg:px-3 mb-2">Order Lists</h1>
      <div className="flex flex-1 flex-col">
        <div className="@container/main flex flex-1 flex-col gap-4 lg:px-3">
          <div>
            <div className="flex flex-col items-start gap-8">
              <ButtonGroup className="">
                <Button variant="outline" size="lg" className="md:p-8">
                  <LuFilter />
                </Button>
                <Button variant="outline" size="lg" className="md:p-8">
                  Filter By
                </Button>
                <Button variant="outline" size="lg" className="md:p-8">
                  Date
                  <IoIosArrowDown />
                </Button>
                <Button variant="outline" size="lg" className="md:p-8">
                  Order Type
                  <IoIosArrowDown />
                </Button>
                <Button variant="outline" size="lg" className="md:p-8">
                  Order Status
                  <IoIosArrowDown />
                </Button>
                <Button
                  variant="outline"
                  size="lg"
                  className="md:p-8 text-red-600"
                >
                  <RiResetRightFill />
                  Reset Filter
                </Button>
              </ButtonGroup>
            </div>
          </div>
          <DataTable data={data} columns={columns} />
          <div className="@container/main grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {/* <ProductSaved /> */}
          </div>
        </div>
      </div>
    </div>
  );
}
